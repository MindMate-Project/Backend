...
exports.registerUser = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    if (role === "patient") {
        user = await User_1.Patient.create({ name, email, password, dateOfBirth, medicalNotes, role: 'patient' });
    }
    else if (role === "caregiver") {
        user = await User_1.Caregiver.create({ name, email, password, relation, phone, role: 'caregiver' });
    }
    else {
