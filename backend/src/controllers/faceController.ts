...
export const registerPatientFace = asyncHandler(
  async (req, res: Response) => {
    ...
    if (knownPerson) {
      formData.append(
        "old_embedding",
        JSON.stringify(knownPerson.average_embedding)
      );
      formData.append(
        "count",
        String(knownPerson.embeddings_count)
      );

      pythonResponse = await axios.post(
        `${PYTHON_AI_URL}/face/update-average`,
        formData,
...
export const identifyPatientByFace = asyncHandler(
  async (req, res) => {
    ...
    const candidates = patient.known_people.map((p: any) => ({
